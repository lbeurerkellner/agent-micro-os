...
async def test_find_from_root(temp_db):
    ...
    with SystemContext(user="find_test_user", fsimage=temp_db) as ctx:
        vault = ctx.fs()
...
async def test_find_from_subdirectory(temp_db):
    ...
    with SystemContext(user="find_sub_user", fsimage=temp_db) as ctx:
        vault = ctx.fs()
...
async def test_find_relative_to_cwd(temp_db):
    ...
    with SystemContext(user="find_cwd_user", fsimage=temp_db) as ctx:
        vault = ctx.fs()
...
async def test_find_empty_directory(temp_db):
    ...
    with SystemContext(user="find_empty_user", fsimage=temp_db) as ctx:
        vault = ctx.fs()
...
async def test_find_sorted_output(temp_db):
    ...
    with SystemContext(user="find_sort_user", fsimage=temp_db) as ctx:
        vault = ctx.fs()

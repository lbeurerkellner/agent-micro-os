...
async def test_rm_single_file(temp_db):
    ...
    with SystemContext(user="rm_test_user", fsimage=temp_db):
...
async def test_rm_directory_without_flag(temp_db):
    ...
    with SystemContext(user="rm_dir_user", fsimage=temp_db):
        vault = SystemContext.current().fs()
...
async def test_rm_absolute_path(temp_db):
    ...
    with SystemContext(user="rm_path_user", fsimage=temp_db):
        vault = SystemContext.current().fs()
...
async def test_rm_permanent_deletion(temp_db):
    ...
    with SystemContext(user="rm_permanent_user", fsimage=temp_db):
        vault = SystemContext.current().fs()
